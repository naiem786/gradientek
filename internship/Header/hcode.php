...
if(isset($_POST['header'])){
    ...
    $query="UPDATE header SET email='$email', phone='$phone' WHERE id='1'";
...
// phone logo
    ...
    if($image != NULL)  
        ...
        // code for inserting data
    $query="UPDATE header SET image='$update_filename',pimage='$pupdate_filename' WHERE id='1'";
    $query_run =mysqli_query($con,$query);

    if($query_run){
        if($image != NULL){

            if(file_exists('../images/logo/header/'.$pfilename)){
                unlink("../images/logo/header/'.$pfilename");
            }
            move_uploaded_file($_FILES['pimage']['tmp_name'],'../images/logo/header/'.$pupdate_filename);
        }
        if($image != NULL){
